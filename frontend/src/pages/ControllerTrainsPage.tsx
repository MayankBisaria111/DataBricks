...
import { TrainTable } from "../components/dashboard";
import { useDarkMode } from "../contexts/DarkModeContext";
import { controllerStations, controllerTrains } from "./controllerData";

export default function ControllerTrainsPage() {
  const { isDark } = useDarkMode();
  const [selectedStation, setSelectedStation] = useState("Delhi Central");
  const rows = useMemo(() => controllerTrains.filter((train) => train.station === selectedStation), [selectedStation]);

  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="space-y-6">
      <div>
        <h1 className={`text-3xl font-semibold tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>Controller Trains</h1>
        <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>Operational train list by station.</p>
      </div>
